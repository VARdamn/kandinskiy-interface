...
export const ImageGenerator = () => {
	...
	const handleGenerate = async () => {
		...
		try {
			...
			if (!uuid) throw Error('Invalid response')

			const interval = setInterval(async () => {
				try {
					const statusRes = await fetch(`http://localhost:5000/api/images/status?uuid=${uuid}`, {
						method: 'GET',
					});
			...
			<div className="flex-none w-[500px] space-y-6">
				...
				<CardContent className="space-y-4 p-6">
					...
					<Textarea 
						...
						placeholder="Например, пушистый кот в очках" 
					/>
					...
					<div className="flex items-center gap-2 mb-2">
						...
						<Tooltip>
							...
							<TooltipContent>
							<p>Что <b>не</b> должно быть на изображении. Например: "кислотные цвета, шум, артефакты".</p>
							</TooltipContent>
						</Tooltip>
						</TooltipProvider>
					</div>
					<Textarea 
						...
						placeholder='Например, яркие цвета'
					/>
					</div>
